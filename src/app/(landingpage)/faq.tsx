...
const Faq = () => {
  ...
  return (
    <div className="pt-32 p-4  md:p-24">
       <div className="absolute bottom-0 left-0 right-0 top-0 bg-[radial-gradient(circle_350px_at_10%_5200px,rgba(213,197,255,.25),transparent)]">
        <div className="absolute bottom-0 left-0 right-0 top-0 bg-[radial-gradient(circle_350px_at_100%_5200px,rgba(186,255,236,.25),transparent)]">
          <div className="absolute bottom-0 left-0 right-0 top-0 bg-[radial-gradient(circle_350px_at_100%_5800px,rgba(244,233,193,.25),transparent)]">
            <div className="absolute bottom-0 left-0 right-0 top-0 bg-[radial-gradient(circle_350px_at_10%_5800px,rgba(194,244,230,.3),transparent)]"></div></div></div></div>
      <h1
      ...
      <div className="max-w-3xl mx-auto text-center">
        {faqs.map((faq, index) => (
          <motion.div
            ...
            className="mb-4 p-4 sm:p-6 bg-slate-200 rounded-md shadow-sm cursor-pointer"
            data-aos="fade-up"
            data-aos-delay={`${index * 100}`}
            aria-expanded={selectedId === faq.id}
            role="button">
            <p className="text-md sm:text-lg font-medium">{faq.question}</p>
            <span className="text- text-slate-900 text-xl flex justify-center  mt-2">
              {selectedId === faq.id ? "-" : <ChevronDown/>}
            </span>
            {selectedId === faq.id && (
              ...
              {faqs
                ...
                .map((filteredFaq) => (
                  <motion.div key={filteredFaq.id}>
                    <motion.h5 className="text-lg font-medium border-b p-2 border-slate-300">
